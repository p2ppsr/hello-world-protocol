...
module.exports = async ({ db, req, res }) => {
  try {
    ...
    */
    const pipeline = [
      {
        $replaceRoot: {
          newRoot: {
            $mergeObjects: [
              '$fullDocument',
              { _id: '$_id', _busdriver_original_id: '$fullDocument._id' }
            ]
          }
        }
      }
    ...
    /*
      Since we've changed the _id field into _busdriver_original_id, we will now modify the find and project operations such that they are referencing the correct value.
    */
    if (typeof parsedQuery.q.find._id !== 'undefined') {
      parsedQuery.q.find._busdriver_original_id = parsedQuery.q.find._id
    ...
    if (parsedQuery.q.project) {
      if (typeof parsedQuery.q.project._id !== 'undefined') {
        parsedQuery.q.project._busdriver_original_id = parsedQuery.q.project._id
        delete parsedQuery.q.project._id
      }
      pipeline.push({ $project: parsedQuery.q.project })
    }
    const cursor = await db
      .collection('busdriver_events')
      .watch(pipeline)
    ...
    cursor.on('change', event => {
      /*
        We will now rename the _busdriver_original_id field back to _id before
      ...
      */
      event._id = event._busdriver_original_id
      delete event._busdriver_original_id
      res.write(
        'data: ' +
        JSON.stringify({
          type: 'message',
